...
檢查你現在是不是在這套工具自己的原始碼目錄，而不是別人拿到的副本。按順序做你權限允許的檢查：

1. 當前目錄有沒有 `.git`，且 remote 指向本專案的上游？
2. 當前路徑裡有沒有 `docs/`、`dist-src/`、`open_source_prep/` 這些片段？
3. 有沒有 `kit/.dev-tree` 這個標記檔？

**回報你實際跑了哪幾項、哪幾項因為權限做不到。** 說「第 2、3 項跑了，第 1 項超出我的權限範圍」是正確答案；**默默跳過然後說一切正常，不是。**

任何一項命中 → 停下來說明，不要裝，也不要「好心」改裝到別的地方。
...
4. **執行** — 跑 `kit/install.ps1`，把路徑跟名字當參數傳進去
...
`SOUL.md` 裝好時是空的樣板，裡面有三段待填：**我是誰 / 我的人是誰 / 我們的關係**。
...
把他的回答整理進去，寫完**唸一遍給他確認**。三段有任何一段還留著「（待填）」，就告訴他這台主腦還沒真正啟用。
